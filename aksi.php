...
elseif ($mod == 'diagnosa_tambah') {
    ...
    elseif ($db->get_row("SELECT * FROM tb_diagnosa WHERE kode_diagnosa='$kode'"))
        ...
        $db->query("INSERT INTO tb_diagnosa (kode_diagnosa, nama_diagnosa, keterangan, referensi) VALUES ('$kode', '$nama', '$keterangan', $referensi)");
    ...
    if ($kode == '' || $nama == '')
        ...
        $db->query("UPDATE tb_diagnosa SET nama_diagnosa='$nama', keterangan='$keterangan', referensi='$referensi' WHERE kode_diagnosa='$_GET[ID]'");
        redirect_js("index.php?m=diagnosa");
    }
...
/** relasi */
    ...
    $kode_user = esc_field($_POST['kode_user']);
    $nama = esc_field(_post('nama'));
    $username = esc_field(_post('username'));
    $password =esc_field(_post('password'));
    $level = esc_field(_post('level'));

    $db->query("INSERT INTO tb_admin (kode_user, nama_user, user, pass, level) VALUES ('$kode_user','$nama', '$username', '$password', '$level')");
    ...
    $db->query("UPDATE tb_admin SET nama_user='$nama', user='$username', pass='$password', level='$level' WHERE kode_user='$kode_user'");
